(ns marketentry.marketentryllm
  ...
  the Democratic Socialist Republic of Sri Lanka (LKA) public-sector
  ...
  cited), never a committed record or a real NPC-governed bid/tender
  response or BOI investment-registration application. Every output is
  censored downstream by `marketentry.governor` before anything touches
  the SSoT, and `:filing/draft`/`:filing/submit` proposals NEVER
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world NPC-governed bid/tender
  response or BOI investment-registration application submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け提出提案"
     ...
     :rationale  (if e
                   (str "tin-registered?=" (:tin-registered? e)
                        " sliced?=" (:sliced? e)
                        " parent-procurement-id=" (:parent-procurement-id e)
