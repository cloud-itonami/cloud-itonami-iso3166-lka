...
(deftest clean-intake-auto-commits
  (let [[db actor] (fresh)
    ...
    (is (= "Colombo Trading House (Pvt) Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest slicing-lacks-justification-is-held-and-unoverridable
  (testing "Procurement Guidelines - 2024 s.4.3(a) -- sliced engagement with no slicing-justification on file -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:slicing-evades-classification} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest slicing-sum-mismatch-is-held-and-unoverridable
  (testing "Procurement Guidelines - 2024 s.4.3(c) -- declared-tce for the sole slice on file falls far short of the group's own declared aggregate-tce -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
          _ (assess! actor "t6pre" "eng-5")
          _ (draft! actor "t6pre" "eng-5")
          res (exec-op actor "t6" {:op :filing/submit :subject "eng-5"} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:slicing-evades-classification} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
...
(deftest tin-registration-unverified-is-held-and-unoverridable
  (testing "unverified Inland Revenue Department TIN registration when required -> HARD hold"
    (let [[db actor] (fresh)
...
(deftest non-sliced-engagement-is-not-gated-by-slicing-check
  (testing "eng-1 is not declared as a slice (:sliced? false) -- the flagship check never fires for it, because Procurement Guidelines - 2024 s.4.3 only speaks to a procurement the PE itself chose to slice/package"
    (let [[db actor] (fresh)
          _ (assess! actor "t9pre" "eng-1")
          _ (draft! actor "t9pre" "eng-1")
          res (exec-op actor "t9" {:op :filing/submit :subject "eng-1"} operator)]
      (is (= :interrupted (:status res)) "clean submit still escalates for human approval, not held")
      (let [r2 (approve! actor "t9")]
        (is (= :commit (get-in r2 [:state :disposition])))))))

(deftest properly-justified-reconciled-slice-is-not-gated
  (testing "eng-7/eng-8 are a properly-justified pair of slices of the SAME parent-procurement-id whose declared-tce sums to the group's own declared aggregate-tce -- the flagship check does not fire, submit still escalates for human approval like any clean submit"
    (let [[db actor] (fresh)
          _ (assess! actor "t10pre" "eng-7")
          _ (draft! actor "t10pre" "eng-7")
          res (exec-op actor "t10" {:op :filing/submit :subject "eng-7"} operator)]
      (is (= :interrupted (:status res)) "clean submit still escalates for human approval, not held")
...
(deftest submit-always-escalates-then-human-decides
  (testing "a clean fully-assessed submit still ALWAYS interrupts for human approval"
    (let [[db actor] (fresh)
          ...
          r1 (exec-op actor "t11" {:op :filing/submit :subject "eng-1"} operator)]
...
(deftest engagement-double-draft-is-held
  (testing "drafting the same engagement twice -> HOLD on the second attempt"
    (let [[db actor] (fresh)
          ...
          res (exec-op actor "t13" {:op :filing/draft :subject "eng-1"} operator)]
      (is (= :hold (get-in res [:state :disposition])))
...
(deftest engagement-double-submit-is-held
  (testing "submitting the same engagement twice -> HOLD on the second attempt"
    (let [[db actor] (fresh)
          ...
          _ (exec-op actor "t14a" {:op :filing/submit :subject "eng-1"} operator)
